...
local function update_widget(widget, device)
   ...
   if device.state == UP.DeviceState.FULLY_CHARGED then
      msg = msg .. " Device fully charged"
   end
   if device.state == UP.DeviceState.PENDING_CHARGE then
      msg = msg .. " Device pending charge"
   end
